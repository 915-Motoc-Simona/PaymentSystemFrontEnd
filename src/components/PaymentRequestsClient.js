...
export default function PaymentRequestsClient() {
  ...
  useEffect(()=>{
    if(JSON.parse(localStorage.getItem("user"))){
        fetch(`http://localhost:8080/payments/requests/client`, {
        method: 'GET',
        headers:  new Headers({
...
const handleCancel = async (event) => {
  ...
  return (
    <>
    <Box sx={{ '& > :not(style)': {width: '30ch', flexGrow: 2, height: '50ch'},}}>
    ...
    <TableContainer>
      ...
      <TableHead>
          <TableRow>
            ...
            <StyledTableCell>User who asked</StyledTableCell>
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={row.reference}>
              ...
              <TableCell>
                <Button variant="outlined" color='success' onClick={() => {handleOK(row)}}> AGREE </Button> 
